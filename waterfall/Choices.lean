...
public def first (choices : Choices α) : Choices α := fun visit => do
...
public def collect (choices : Choices α) : Lean.Elab.Tactic.TacticM (Array α) := do
  let values ← IO.mkRef #[]
